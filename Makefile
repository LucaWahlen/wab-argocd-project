...
build-api:
	tar -C apps/shipping-cost-api -cf - . | ssh ubuntu@$(VM_IP) "mkdir -p ~/wab-api && tar -xf - -C ~/wab-api"
	ssh ubuntu@$(VM_IP) "cd ~/wab-api && sudo podman build -t localhost/shipping-cost-api:$(API_VERSION) . && sudo env KIND_EXPERIMENTAL_PROVIDER=podman kind load docker-image localhost/shipping-cost-api:$(API_VERSION) --name wab"
...
reset-cluster:
	ssh ubuntu@$(VM_IP) "sudo kind delete clusters --name wab && sudo rm -f /root/.kube/config"
	$(MAKE) bootstrap
